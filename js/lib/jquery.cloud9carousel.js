...
;(function($) {
  ...
  var transform = (function() {
    var jss = ['webkit', 'moz', 'ms'];
    var style   = document.createElement( "div" ).style;
    var trans   = 'transform' in style ? 'transform' : undefined;

    for( var i = 0, count = jss.length; i < count; i++ ) {
      var prop = jss[i] + 'Transform';
      if( prop in style ) {
        trans = prop;
        break;
      }
  ...
  (function() {
    ...
    for( var i = 0, count = jss.length; i < count && !cancelFrame; i++ ) {
      cancelFrame = window[jss[i]+'CancelAnimationFrame'] || window[jss[i]+'CancelRequestAnimationFrame'];
      requestFrame = requestFrame && window[jss[i]+'RequestAnimationFrame'];
    }
  }());
